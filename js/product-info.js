...
function showComments(commentInfo) {
  ...
  for (let i = 0; i < commentInfo.length; i++) {
    // recorro todo el array, comentario por comentario
...
function addComments() {
  ...
  let formatDate =
    ...
    "-" +
    date.getDate().toString().padStart(1, "0") +
    " " +
  ...
  for (i = 0; i < selected.length; i++) {
    if (selected[i].checked) {
      // filtra el botón apretado
      score = selected[i].value; // le asigna a score el valor del botón apretado
    }
  }
  if (message.trim() != "") {
    // Para que no admita comentarios vacíos
    comment = {
      // armo el nuevo objeto comment
      description: message,
      dateTime: formatDate,
      score: score,
...
function showProductInfo(productInfo) {
  ...
  let htmlInfo =
    ...
    `</p>
      <div class="col-12">
        <div class="row">
          <div class="col-5">
            <a class="badge badge-tomato" style="font-size:1.3em" href="cart.html">` +
    ...
    ` </small>
          ...
          <div class="col-4" style="text-align:right">
            <small class="text-muted"> Categoría: <a href="category-info.html">` +
  ...
  for (let i = 0; i < productInfo.images.length; i++) {
    ...
    if (i === 0) {
      // Hago por separado el primero que lleva la clase active
      htmlLista +=
      ...
      <div class="carousel-item active">
        <img class="d-block w-100 rounded" src=` +
        image +
        ` alt="First slide">
      </div>`;
    } else {
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      for (
        var i = 0;
        i < ele.length;
        i++ // recorre todas las opciones de botones radio que hay con el nombre estrellas
      )
        ele[i].checked = false;
    });
  // al apretar el botón de limpiar el comentario y puntuación
  document
    .getElementById("clean-comment")
    .addEventListener("click", function () {
      document.getElementById("textarea").value = ""; // Para limpiar donde se escribe
      var ele = document.getElementsByName("estrellas"); // Para limpiar la selección de puntos
      for (
