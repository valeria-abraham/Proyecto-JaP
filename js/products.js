...
function search() {
  //es la función para hacer la búsqueda en la barra
  var textInput = document.getElementById("search").value; //obtengo el texto introducido
  filteredList = currentProductsArray.filter(function (product) {
    //uso la función que filtra automáticamente y selecciono de dónde quiero buscar los caracteres introducidos
    return (
      product.name.toLowerCase().indexOf(textInput.toLowerCase()) > -1 ||
      product.description.toLowerCase().indexOf(textInput.toLowerCase()) > -1
    );
  });
