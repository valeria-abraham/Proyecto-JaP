...
function showName() {
  //función que muestra el nombre en la barra de navegación de la página
  ...
  if (document.getElementById("mi-nombre") != null) {
    //solo muestra si está presente la barra de navegación arriba
    document.getElementById("mi-nombre").innerHTML = htmlContentToAppend;
  }
}

function isConnected() {
  //esta función verifica que se haya hecho el inicio de sesión
  let nombre = localStorage.getItem("Nombre");
  if (nombre != undefined || nombre != null) {
    showName(); //si estoy conectada muestro mi nombre de usuario en la barra de navegación
  } else if (window.location.pathname.slice(-10) != "index.html") {
    //veo si no estoy ya en la página de login
    window.location.href = "index.html"; //si no estoy conectada mando a la página de login
  }
}

function logOut() {
  // función que desconecta al usuario borrando toda la información del localStorage
